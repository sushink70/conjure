...
class GraphVisualizer:
    def __init__(self, graph: Dict, start_node: Any, algorithm: str = "bfs", delay: float = 0.5, no_clear: bool = False):
        self.graph = graph
...
class TreeVisualizer:
    def __init__(self, tree_dict: Dict, root: Any, algorithm: str = "bfs", delay: float = 0.5, no_clear: bool = False):
...
Examples:
  ...
  python visualizer.py graph.py --algorithm bfs --start 1 --delay 0.5 --no-clear
        """
    )
    ...
    parser.add_argument("--no-clear", action="store_true",
                       help="Don't clear screen between steps (better for logging)")
    ...
    if structure_type == "tree":
        visualizer = TreeVisualizer(graph, start, args.algorithm, args.delay, args.no_clear)
    else:
        visualizer = GraphVisualizer(graph, start, args.algorithm, args.delay, args.no_clear)
